...
const currentAssessmentYear = computed(() => {
  let years = [];
  let year;
  if (assessmentHistory.value) {
    let values = assessmentHistory.value;
...
const activeModal = computed(() => {
  return MainStore.activeModal;
});

const activeFeatureId = computed(() => {
  return activeModal.value.featureId;
});
...
const activeAddress = computed(() => {
  ...
  console.log('activeAddress computed is running, address:', address, 'activeModalFeature.value: ', activeModalFeature.value, 'lastSearchMethod:', lastSearchMethod.value);
  return address;
});
...
const propValueCalloutSlots = computed(() => {
  return {
    text: '\
...
const localDetailsVerticalTableSlots = computed(() => {
  ...
  let secondaryTrashDay = function(MainStore) {
    let prop = MainStore.activeModalFeature.properties ? state.activeModalFeature.properties : MainStore.activeModalFeature
    ...
    fields: [
      {
        ...
        value: function(state) {
          return "<a href='http://atlas.phila.gov/"+activeAddress.value+"/voting' target='_blank'>\
      ...
      },
      {
        ...
        value: function() {
          ...
          let value = '';
          console.log('secondaryTrashDay(state): ', secondaryTrashDay(MainStore));
          if (!secondaryTrashDay(MainStore)) {
            value = trashDay(MainStore);
          } else {
            let days = [ 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday' ];
            let i = 0;
            for (let day of days)
              if (day === trashDay(MainStore) || day === secondaryTrashDay(MainStore)) {
              i += 1;
              value += day;
              if (i < 2) {
                value += ' and ';
...
const propertyDetailsVerticalTableSlots = computed(() => {
  // console.log('PropertyCard activeFeatureId computed is running')
  let state = MainStore;
  let opaPublicData = activeOpaData.value;
  // let opaPublicData = state.sources.opa_public.targets[activeOpaId.value].data;
  ...
  return {
    ...
    <a target="_blank" href="https://opainquiry.phila.gov/opa.apps/help/PropInq.aspx?acct_num='+ activeOpaId.value + ' ">\
    ...
    fields: [
        ...
        value: function(state) {
          ...
          if (opaPublicData.zoning) {
            return '<a target="_blank" \
                      href="https://atlas.phila.gov/'+ encodeURIComponent(activeAddress.value) + '/zoning ">\
      ...
      },
      {
        ...
        value: activeOpaId.value,
      },
      {
        label: 'OPA Address',
        value: titleCase(activeAddress.value),
      },
      {
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        value: function(item) {
          let mailingAddress = [];
          mailingAddress.push('<span class="large-owner">' + activeOpaId.value+'</span><br>');
          ...
          if(item.mailing_city_state !== null) {
            ...
            // console.log('activeAddress', activeAddress.value, item)
            let zip = item.zip_code.substring(0,5) + '-' + item.zip_code.substring(5,10);
            mailingAddress.push(titleCase(activeAddress.value), '<br>Philadelphia, PA', zip);
            return mailingAddress.join(' ');
          }
...
const valuationHistoryHorizontalTableOptions = computed(() => {
  return {
    id: 'ownerProperties',
    tableid: 'ddd',
    // dataSources: ['opa'],
...
const salesHistoryHorizontalTableOptions = computed(() => {
  return {
    id: 'salesHistory',
    tableid: 'ddd',
    // dataSources: ['opa'],
...
watch(
  ...
  if (nextActiveAddress) {
    ...
    MainStore.setActiveAddressKnown(false);
  }
});
...
const buttonLinkLI = () =>{
  window.open('https://li.phila.gov/property-history/search?address=' + activeOpaId.value, '_blank');
  return false;
};

const buttonLinkTaxBalance = () => {
  window.open('https://tax-services.phila.gov/TAP/EWebServices/realestate/search/' + activeOpaId.value, '_blank');
  return false;
};
